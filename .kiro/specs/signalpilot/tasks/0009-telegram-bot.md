...
- [x] `format_signal_message(signal: FinalSignal) -> str`:
...
- [x] `format_status_message(signals: list, trades: list, market_data: MarketDataStore) -> str`:
...
- [x] `format_journal_message(metrics: PerformanceMetrics) -> str`:
...
- [x] `format_daily_summary(summary: DailySummary) -> str`:
...
- [x] Write tests in `tests/test_telegram/test_formatters.py`:
...
- [x] `handle_taken(signal_repo, trade_repo, exit_monitor, update)`:
...
- [x] `handle_status(signal_repo, trade_repo, market_data, update)`:
...
- [x] `handle_help(update)`:
  - Respond with formatted list of commands and descriptions (Req 26.1)
- [x] Write tests in `tests/test_telegram/test_handlers.py` with mocked repos and update context:
...
- [x] Implement `SignalPilotBot` class as specified in design (Section 4.6.1)
- [x] `__init__(bot_token, chat_id, dependencies)`:
...
- [x] `stop()`: gracefully stop updater, application, and shutdown
- [x] `send_signal(signal: FinalSignal)`:
...
- [x] `send_alert(text: str)`: send plain text alert to chat_id
- [x] `send_exit_alert(alert: ExitAlert)`:
  - Format using `format_exit_alert()`
  - Send to chat_id
- [x] Write tests in `tests/test_telegram/test_bot.py` with mocked Application:
...
All subtasks implemented and tested. 44 tests passing (5 bot, 22 formatters, 12 handlers, 5 bot integration).

### Code Review Summary

**Fixes applied:**
- C2: Added `filters.Chat(chat_id=int(self._chat_id))` to all handlers for chat ID authorization
- H1: Added `parse_mode="HTML"` to `_handle_status` and `_handle_journal` replies
- H4: Changed `datetime.now()` to `datetime.now(IST)` in handlers.py
- M1: Added input clamping to `star_rating()`: `strength = max(1, min(5, strength))`
- M2: Aligned CAPITAL regex in bot.py to `r"(?i)^capital\s+\d+(?:\.\d+)?$"`
- M5: Changed hardcoded "BUY SIGNAL" to use `c.direction.value`

**Deferred (low risk, single-user system):**
- C1: Race condition in handle_taken (single-user, no concurrency)
- C3: ZeroDivisionError guard in formatters (upstream validates entry_price > 0)
- H2: No try/except in handlers (python-telegram-bot error handler covers this)
- H3: No upper bound on CAPITAL amount (single-user)
- H5: No null check on `_application` before use
